...
class MultiTorrent(Feedback):
    ...
    def __init__(self, config, rawserver,
        ...
        self.config = config
        print "config[micropayment_certificate]=%s" % config["micropayment_certificate"]
        print "config[micropayment_private_key]=%s" % config["micropayment_private_key"]
        print "config[micropayment_trusted_ca_dir]=%s" % config["micropayment_trusted_ca_dir"]
    ...
    def finished(self, torrent):
        ...
        if self.config['exit_when_complete']:
            #torrent.logger.debug("exiting because complete")
            print "commented out: exiting because complete"
            #self.shutdown()
        else:
            torrent.logger.debug("not exiting because complete")
