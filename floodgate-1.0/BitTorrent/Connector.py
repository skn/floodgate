...
noisy = True
log_data = False
KEY_LENGTH = 128 #1024bit pub key div by 8bitpbyte
...
#message id, key or no key<len=1+len_key><id=10><index><begin><length><key>
21:'DOWNLOAD_REWARD',

#message id, boolean<len=2><id=11><index><begin><length><boolean>
22:'DOWNLOAD_REWARD_RESPONSE',
...
class Connector(Handler):
    ...
    def send_request(self, index, begin, length):
        ...
        message = pack("!ciii", REQUEST, index, begin, length)
        #if self.download.multidownload.micropayments:
            #key = self.download.multidownload.private_key
            #print "generating signature with key: "+str(key)
            #signature = self.download.multidownload.pk_tools.get_sha_signature(self.download.multidownload.private_key,message)
            #message += signature
        self._send_message(message)
        
    def send_mp_request(self, index, begin, length,sig,certificate):
           
         cert_len = len(certificate)
         sig_len = len(sig)
         if noisy:
            log("SEND %s %d %d %d cl %d sigl %d" % (message_dict[MP_REQUEST], index, begin, length,cert_len,sig_len)) 
         message = pack("!ciiiii%ds%ds" % (cert_len,sig_len), MP_REQUEST, index, begin, length,cert_len,sig_len,certificate,sig)
         self._send_message(message)
    ...
    def send_partial(self, bytes):
        ...
        if self._partial_message is None:
            ...
            while self.upload.buffer and buf.tell() < bytes:
                if not self.upload.multidownload.micropayments:
                   t, piece = self.upload.buffer.pop(0)
                   index, begin, length = t
                   msg = pack("!icii%s" % len(piece), len(piece) + 9, PIECE,
                              index, begin)
                else:
                   t, piece,sig,cert = self.upload.buffer.pop(0)
                   index, begin, length,len_sig, len_cert = t
                   msg_len = len(piece) +len(sig)+len(cert)+ 21
                   msg = pack("!iciiiii", 
                               msg_len, MP_PIECE, index, begin,length,len_sig,len_cert)
                   log("SEND PIECE idx %d begin %d len %d lensig= %d len_cert = %d" % (index, begin,length,len_sig,len_cert))
                   piece = piece+sig+cert 
                buf.write(msg)
                buf.write(piece)
    ...
    def _got_message(self, message):
        ...
        if t == CHOKE:
            ...
            i, a, b,cert_len,sig_len = unpack("!xiiiii", message[:21])
            if noisy: log("GOT MP_REQUEST %d %d %d %d %d" % (i, a, b,cert_len,sig_len))
            if i >= self.parent.numpieces:
                self.protocol_violation(
                     "Requested piece index out of range: %d > %d" %
                     (i, self.parent.numpieces))
                self.close()
                return
            (der_cert,sig) = unpack("%ds%ds" % (cert_len,sig_len), message[21:])
            self.upload.got_mp_request(i, a, b,der_cert,sig)
            return
        ...
        elif t == DOWNLOAD_REWARD:
            if noisy: log("GOT %s" % message_dict[t])
            #<len=1+len_key><id=10><index><begin><len_blk><key>
            #<len=1+20+4+4+4=33><id=10><index><begin><len_blk><key>
            
            if len(message) != 1+4+4+4+KEY_LENGTH:
               self.protocol_violation("KEY_REWARD length %d != 33" %
                                       len(message))
               self.close()
               return
            format_string =  "!xiii"
            i, o, l = unpack(format_string,message[:13]) #x skips msg_type,
            k = message[13:]
            if noisy: log("GOT %s idx=%d offset=%d len=%d key-len=%d" % (message_dict[t],i,o,l,len(k)))
            self.upload.got_key_reward(i, o, l,k)
        elif t == DOWNLOAD_REWARD_RESPONSE:
            if noisy: log("GOT %s" % message_dict[t])
             #<len=1+4+4+4+1><id=11><index><begin><len_blk><key>
            if len(message) != 14:
                print("wrong length %d" % len(message))
                self.protocol_violation("KEY_REWARD_RESPONSE length %d != 33" %
                                       len(message))
                self.close()
                return
            log("calling download.got_key_reward_response ")
            i, o, l, r = unpack("!xiiiB",message)
            if noisy: log("GOT %s idx=%d offset=%d len=%d result = %d" % (message_dict[t],i,o,l,r))
            self.download.got_key_reward_response(i,o,l,r)
        elif t == PIECE:
            if len(message) <= 9:
            ...
            if len(message) <= 21: #format = ciiiii, 5 ints + 1 char
                self.protocol_violation("MP_PIECE %d != 9" %
                                        len(message))
                self.close()
                return
            
            header = message[:21]
            header = unpack("!xiiiii",header)
            index,offset,piece_len,sig_len,cert_len = header
            log ("mp header info: idx=%d offset=%d pieclen=%d siglen=%d certlen=%d" % (index,offset,piece_len,sig_len,cert_len))
            log("piecelen+siglen+certlen= %d, actual length=%d" % (piece_len+sig_len+cert_len, len(message[21:])))
            piece,sig,der_cert = unpack("%ds%ds%ds" % (piece_len,sig_len,cert_len), message[21:])

            if noisy: log("GOT MP_PIECE %d %d" % (index, offset))
            if index >= self.parent.numpieces:
                self.protocol_violation("PIECE %d >= %d" %
                                        (i, self.parent.numpieces))
                self.close()
                return
            self.download.got_cert(der_cert)
            self.download.got_mp_piece(index, offset, piece, sig)   
    ...
    def data_came_in(self, conn, s):
        ...
        while True:
            ...
            try:
                ...
                self.logger.exception("connector stopiteratrion")
                self.close()
                return
            except:
 ...
 #message id, key or no key<len=1+len_key><id=10><index><begin><length><key>
    """
    send a key reward for a successfully received piece
    only applicable to micropaid torrents
    
    @param piece_number: The number of the piece the reward is for
    @param begin:        The offset in the piece (always zero)
    @param len:         The length of the key
    @param key:        The key to send as the reward
    """
    
    
    def send_key_reward(self, piece_number,begin,len,key):
        
        msg_len = 1+4+4+4 + KEY_LENGTH;                                 #one char msgType, 3 ints
        format_string = "!ciii%ss" % KEY_LENGTH                        #key is included as string of length KEY_LENGTH=20
        msg = pack(format_string, DOWNLOAD_REWARD, piece_number,begin,len,key)  
        self._send_message(msg)
     
 
    
    """
    send a reply to the sender of a keyreward to inform him about:
       success: successful validation of the key
    or failure: key didn't validate or no key received 
        ("no key received" happens if the downloader keeps requesting  and downloading
        pieces without sending rewards)  
    
    @param piece_number: The number of the piece the reward was for
    @param begin:        The offset in the piece (always zero)
    @param len:         The length of the key
    @param key:        The key to send as the reward
    """
    
    def send_key_reward_response(self, piece_number,begin,len,result):
        if result == True:
            result = 1                                                #any bit set indicates succes, for good measure, set all bits
        else:
            result = 0                                                #all bits must be cleard to indicate failure
        msg = pack("!ciiiB", DOWNLOAD_REWARD_RESPONSE, piece_number,begin,len,result)
        self._send_message(msg)