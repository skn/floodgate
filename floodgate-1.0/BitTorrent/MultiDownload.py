...
from tlslite.api import *
import logging
logger = logging.getLogger("BitTorrent.MultiDownload")
log = logger.debug
...
class MultiDownload(object):
    ...
    def __init__(self, config, storage, rm, urlage, picker, numpieces,
                 finished, errorfunc, kickfunc, banfunc, get_downrate,micropayments=False):
        self.config = config
        ...
        self.waiting_for_reward = {} #hash with first key peerid then [(idx1,offset1,len1),(idx2,offset2,len2) ]
        self.payment_key_hash_cache = {} #stores payment key hashes received from tracker
        ...
        if micropayments:
            ...
            self.certificate = parse_PEM_certificate(open(self.config["micropayment_certificate"]))
            self.public_key = self.certificate.publicKey
            self.private_key = parse_PEM_private_key(open(self.config["micropayment_private_key"]))
            log( "cert tracker filename=%s:" % (self.config["micropayment_tracker_certificate"]))
            cert_tracker = parse_PEM_certificate(open(self.config["micropayment_tracker_certificate"]))
    ...
    def parse_key_rewards(self,key_list):
        ...
        if self.key_rewards == {} or self.key_rewards == None:
            ...
            for idx in range(0,keylist_length): 
                ...
                self.key_rewards[(idx,0,piece_size)]=key_list[start:(start+P_KEY_LENGTH)]  
                log( "set keynumber %d %d %d %s" % (idx,0,piece_size,key_list[start:(start+P_KEY_LENGTH)] ))     
        return
