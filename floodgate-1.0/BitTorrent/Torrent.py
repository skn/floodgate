...
class Torrent(object):
    ...
    def _initialize(self):
        ...
        md = MultiDownload(self.config, self._storagewrapper, self._rm,
                           ...
                           self._downmeasure.get_rate,self.metainfo.micropayments,)
    ...
    def start_download(self):
        ...
        if not self.metainfo.is_trackerless:
            self._rerequest = Rerequester(self.metainfo.announce,
                ...
                self._connection_manager.ever_got_incoming,
                self._no_announce_shutdown, self._announce_done,
                bool(self._dht_rerequest), self.multidownload.parse_key_rewards,
                self.metainfo.micropayments,self.multidownload.private_key, 
                self.multidownload.certificate, self.multidownload.public_key_tracker, self.multidownload.pk_tools)
