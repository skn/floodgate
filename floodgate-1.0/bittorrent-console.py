...
class HeadlessDisplayer(object):
    ...
    def finished(self):
        ...
        self.logger.debug("bittorrent console finished")
...
# add the handler to the root logger
    ...
    def get_status(self):
        ...
        if self.torrent is not None:
            ...
            fractionDone = status.get('fractionDone')
            if (fractionDone ==1 or self.d.done == True)and self.shutting_down == False and self.config['exit_when_complete'] == True :
                self.d.error("trying to shutdown")
                df = self.multitorrent.shutdown()
                stop_rawserver = lambda *a : self.multitorrent.rawserver.stop()
                df.addCallbacks(stop_rawserver, stop_rawserver)
